...
async function initialize() {
  try {
    ...
    await initializeEnterKeyHandler();
  } catch (error) {
    console.error("Failed to initialize:", error);
  }
}

// Add new function to handle Enter key
async function initializeEnterKeyHandler() {
  const settings = await getSettings();
  if (settings.preventEnterPropagation) {
      ...
      (event) => {
        if (event.key === "Enter" && !event.shiftKey) {
          ...
          if (editor && editor.contains(event.target)) {
            console.log("Editor found and event target is inside");
            event.stopPropagation();
