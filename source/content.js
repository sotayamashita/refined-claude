...
function createTemplateButton() {
	...
	button.dataset.testid = 'template-selector-dropdown';

	const innerDiv = document.createElement('div');
	innerDiv.className = 'inline-flex items-center min-w-0';
	innerDiv.dataset.state = 'closed';
  ...
  `;
	...
	for (const item of templateItems) {
		item.addEventListener('click', () => {
			const index = Number.parseInt(item.dataset.templateIndex, 10);
...
async function initializeTemplateButton() {
	...
	templateButton.addEventListener('click', async event => {
		...
		dropdownWrapper.dataset.radixPopperContentWrapper = '';
		dropdownWrapper.dataset.templateDropdownWrapper = '';
	...
	innerWrapper.append(templateButton);
	templateWrapper.append(innerWrapper);

	// Add to the end of the target container
	targetContainer.append(templateWrapper);
